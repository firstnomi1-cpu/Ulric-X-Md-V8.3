...
async function generatePairCode(phoneNumber) {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    // CRITICAL FIX for statusCode=515:
    // Baileys' DEFAULT browser identifier ("Ubuntu", "Chrome", "20.0.04")
    // is BLOCKED by WhatsApp on datacenter IPs (Railway, Render, Fly).
    // WhatsApp returns statusCode 515 (stream error) during auth handshake.
    //
    // Solution: Use a UNIQUE random browser identifier per session.
    // This is the ONLY change to makeWASocket config. Everything else
    // (requestPairingCode, saveCreds, auth flow) is identical to the
    // working single-user reference.
    // ═══════════════════════════════════════════════════════════════
    const crypto = require('crypto');
    const prefixes = ['Ulric-X', 'UlricBot', 'UXMD', 'UlricMD', 'UlricSession'];
    const browserPrefix = prefixes[Math.floor(Math.random() * prefixes.length)];
    const browserRandom = crypto.randomBytes(4).toString('hex');
    const uniqueBrowser = [browserPrefix, 'Chrome', '2.0.' + browserRandom];
    console.log(chalk.gray(`[PAIR] Browser ID: ${uniqueBrowser.join(' | ')}`));
    ...
    const sock = makeWASocket({
      ...
      browser: uniqueBrowser,  // ← THE FIX (unique per session)
...
async function startConnection(jid) {
  ...
  try {
    ...
    // Use UNIQUE browser identifier for reconnects too (prevents 515)
    const crypto = require('crypto');
    const prefixes = ['Ulric-X', 'UlricBot', 'UXMD', 'UlricMD', 'UlricSession'];
    const browserPrefix = prefixes[Math.floor(Math.random() * prefixes.length)];
    const browserRandom = crypto.randomBytes(4).toString('hex');
    ...
    const sock = makeWASocket({
      ...
      auth: state,
      printQRInTerminal: false,
      browser: uniqueBrowser,  // ← unique per session (prevents 515)
      connectTimeoutMs: 30000,
      defaultQueryTimeoutMs: 30000,
      keepAliveIntervalMs: 30000,
    });
